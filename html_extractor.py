...
def main():
    ...
    cef.Initialize(settings={"windowless_rendering_enabled": True},# "cef_custom_schemes": ""},
            switches = {"log_severity":"verbose" , "allow_universal_access_from_files":"1", "universal_access_from_file_urls_allowed":"1",
		"disable_gpu":"1", "disable_gpu_composting":"1", "enable_begin_frame_scheduling":"1"} )
    create_browser()
    cef.MessageLoop()
    cef.Shutdown()
...
def command_line_arguments():
	...
	frames=int(sys.argv[4])
        if True or url.startswith("http://") or url.startswith("https://"):
            global URL
            URL = url
        else:
	...
	if frames > 0:
        ...
        print("[screenshot.py] Error: Expected arguments: <url> <width> <height> <frame_count>")
        sys.exit(1)
...
def create_browser():
    ...
    browser = cef.CreateBrowserSync(window_info=window_info,
            settings = {"windowless_frame_rate":60, "web_security_disabled": True},
                                    url=URL)
    browser.SetClientHandler(LoadHandler())
    browser.SetClientHandler(RenderHandler(FRAMES))
...
def save_screenshot(browser,count):
    ...
    image = Image.frombytes("RGBA", VIEWPORT_SIZE, buffer_string,
                            "raw", "RGBA", 0, 1)
    image.save(SCREENSHOT_PATH+"%06d.tga"%count, "TGA")
    print("[screenshot.py] Saved image: {path}".format(path=SCREENSHOT_PATH+"%06d.tga"%count))
